...
class SettingsManager(private val context: Context) {
    ...
    fun getTotalTicketsFlow(): Flow<Int> = dataStore.data
        .map { preferences ->
            (preferences[PreferencesKeys.TOTAL_TICKETS] ?: 0).coerceAtLeast(0)
        }
        .catch { exception ->
            if (exception is IOException) {
                emit(0)
    ...
    val gamePlaySettingsFlow: Flow<GamePlaySettings> = dataStore.data
        ...
        .map { preferences ->
            ...
            GamePlaySettings(
                ...
                haptics = preferences[PreferencesKeys.HAPTICS] ?: true,
